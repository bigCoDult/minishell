...
t_ast_node *parse_pipeline(t_shell *shell, t_token **tokens)
{
    ...
    if (!left)
    {
        debug_print(2013, 8, "DEBUG: Failed to parse left command\n");
        return (NULL);
    }
    ...
    while (*tokens && (*tokens)->type == TOKEN_PIPE)
    {
        ...
        debug_print(2013, 8, "DEBUG: Found pipe token\n");
        *tokens = (*tokens)->next;  // 파이프 토큰 건너뛰기
        
        pipe_node = create_ast_node(shell, AST_PIPE);
        if (!pipe_node)
        {
            debug_print(2013, 8, "DEBUG: Failed to create pipe node\n");
            return (NULL);
        }
        ...
        if (!right)
        {
            debug_print(2013, 8, "DEBUG: Failed to parse right command\n");
            return (NULL);
        }
        ...
        left = pipe_node;
        debug_print(2013, 8, "DEBUG: Successfully created pipe node\n");
    }
