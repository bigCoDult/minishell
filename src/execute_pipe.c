...
static int handle_all_heredocs(t_shell *shell, t_ast_node *node)
{
    ...
    debug_print(2047, 1, "=== HANDLING ALL HEREDOCS ===\n");
    ...
    while (left_redirs)
    {
        ...
        {
            debug_print(2047, 1, "Processing left heredoc: %s\n", left_redirs->filename);
    ...
    while (right_redirs)
    {
        ...
        {
            debug_print(2047, 1, "Processing right heredoc: %s\n", right_redirs->filename);
    ...
    debug_print(2047, 1, "Processed %d heredocs\n", heredoc_count);
...
int execute_pipe(t_shell *shell, t_ast_node *node)
{
    ...
    if (pid1 == 0)
    {
        ...
        {
            debug_print(2047, 1, "Executing left command: %s\n", node->left->cmd->args[0]);
    ...
    debug_print(2047, 1, "Left command status: %d\n", WEXITSTATUS(status1));
    debug_print(2047, 1, "Right command status: %d\n", WEXITSTATUS(status2));
