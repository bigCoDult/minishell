...
t_command *create_command(t_shell *shell, t_token **tokens)
{
    ...
    while (curr && curr->type != TOKEN_PIPE)
    {
        if (curr->type == TOKEN_WORD)
        {
            ...
            {
                debug_print(2013, 7, "Processing word: %s\n", curr->value);
...
char **create_args_array(t_shell *shell, t_token *start, int arg_count)
{
    ...
    debug_print(2013, 3, "DEBUG: [create_args_array] Creating args array with count: %d\n", arg_count);
    args = shell_malloc(shell, sizeof(char *) * (arg_count + 1));
    if (!args)
    {
        debug_print(2013, 3, "DEBUG: [create_args_array] Failed to allocate args array\n");
        return (NULL);
    }
    ...
    {
        debug_print(2013, 3, "DEBUG: [create_args_array] Adding arg[%d]: %s\n", i, start->value);
        args[i] = shell_strdup(shell, start->value);
        if (!args[i])
        {
            debug_print(2013, 3, "DEBUG: [create_args_array] Failed to duplicate argument\n");
            return (NULL);
        }
        start = start->next;
        i++;
    ...
    args[i] = NULL;
    debug_print(2013, 3, "DEBUG: [create_args_array] Args array created successfully\n");
    return (args);
}
...
int parse_input(t_shell *shell)
{
    ...
    {
        debug_print(2013, 3, "DEBUG: [parse_input] Tokenization failed\n");
        return (1);
    }
    debug_print(2013, 3, "DEBUG: [parse_input] Tokenization completed\n");
    ...
    while (curr_token)
    {
        debug_print(2013, 3, "DEBUG: [parse_input] Token type: %d, value: %s\n",
    ...
    if (!shell->ast_root)
    {
        debug_print(2013, 3, "DEBUG: [parse_input] Pipeline parsing failed\n");
        return (1);
    }

    debug_print(2013, 3, "DEBUG: [parse_input] Parsing completed successfully\n");
    debug_print(2013, 3, "DEBUG: [parse_input] === Parsing finished ===\n\n");
    return (0);
}