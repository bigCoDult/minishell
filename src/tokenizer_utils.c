...
static int get_word_length(char *input)
{
    ...
    debug_print(2013, 3, "DEBUG: Calculating word length\n");
    len = 0;
    while (input[len] && !strchr(" |<>", input[len]))
    {
        ...
        debug_print(2013, 3, "DEBUG: Checking character: %c\n", input[len]);
        len++;
    }
    debug_print(2013, 3, "DEBUG: Word length: %d\n", len);
    return (len);
}
...
char *handle_word(t_shell *shell, char *input, int *len)
{
    ...
    debug_print(2013, 3, "DEBUG: Handling word starting with: %c\n", *input);
    
    *len = get_word_length(input);
    if (*len == 0)
    {
        debug_print(2013, 3, "DEBUG: Empty word\n");
        return NULL;
    }
    ...
    if (!word)
    {
        debug_print(2013, 3, "DEBUG: Failed to allocate memory for word\n");
        return NULL;
    }
    ...
    debug_print(2013, 3, "DEBUG: Expanded word: %s\n", expanded);
    return expanded;
}
