...
t_ast_node *create_ast_node(t_shell *shell, t_ast_type type)
{
    ...
    debug_print(2013, 4, "DEBUG: Creating AST node of type: %s\n",
           type == AST_COMMAND ? "COMMAND" : "PIPE");
    node = shell_malloc(shell, sizeof(t_ast_node));
    if (!node)
    {
        debug_print(2013, 4, "DEBUG: Failed to allocate AST node\n");
        return (NULL);
    }
    ...
    node->left = NULL;
    node->right = NULL;
    debug_print(2013, 4, "DEBUG: AST node created successfully\n");
