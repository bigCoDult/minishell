...
int execute_simple_command(t_shell *shell, t_command *cmd)
{
    ...
    debug_print(0, 1, "Command: %s\n", cmd->args[0]);
    debug_print(0, 1, "Current stdin FD: %d\n", STDIN_FILENO);
    
    // 리다이렉션 처리
    t_redirection *redir = cmd->redirs;
    while (redir)
    {
        debug_print(0, 1, "Processing redirection type: %d\n", redir->type);
        debug_print(0, 1, "Redirection filename: %s\n", redir->filename);
        
        // 히어독은 이미 handle_all_heredocs에서 처리되었으므로 건너뜀
        if (redir->type == REDIR_HEREDOC)
        {
            debug_print(0, 1, "Skipping already processed heredoc\n");
            redir = redir->next;
            continue;
        }
    ...
    // 명령어 실행
    debug_print(0, 1, "Executing command: %s\n", cmd->args[0]);
    if (is_builtin(cmd->args[0]))
    {
        debug_print(0, 1, "Executing builtin command\n");
        ret = execute_builtin(shell, cmd);
    }
    else
    {
        debug_print(0, 1, "Executing external command\n");
        ret = execute_external(shell, cmd);
    }

    debug_print(0, 1, "Command execution completed with status: %d\n", ret);
    debug_print(0, 1, "Current stdin FD: %d\n", STDIN_FILENO);
    debug_print(0, 1, "=== EXECUTE SIMPLE COMMAND END ===\n\n");
    return ret;
}

int execute_ast(t_shell *shell, t_ast_node *node)
{
    debug_print(0, 4, "DEBUG: Executing AST node\n");
    
    if (!node)
    {
        debug_print(0, 4, "DEBUG: No node to execute\n");
        return (0);
    }
    ...
    {
        debug_print(0, 4, "DEBUG: Executing command node: %s\n", node->cmd->args[0]);
        return (execute_simple_command(shell, node->cmd));
    }
    else if (node->type == AST_PIPE)
    {
        debug_print(0, 4, "DEBUG: Executing pipe node\n");
        return (execute_pipe(shell, node));
    }

    debug_print(0, 4, "DEBUG: Unknown node type\n");
    return (1);
}
