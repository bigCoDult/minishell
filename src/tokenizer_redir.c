...
static t_token *create_redir_token(t_shell *shell, const char *value, t_token_type type)
{
    ...
    debug_print(2013, 3, "DEBUG: Creating redirection token: %s\n", value);
    token = shell_malloc(shell, sizeof(t_token));
    if (!token)
    {
        debug_print(2013, 3, "DEBUG: Failed to allocate redirection token\n");
        return (NULL);
    }
...
t_token *handle_redirection(t_shell *shell, char **input)
{
    ...
    debug_print(2013, 10, "DEBUG: [handle_redirection] Processing redirection\n");
    ...
    if (**input == '<' && *(*input + 1) == '<')  // heredoc
    {
        ...
        *input += 2;
        ...
        if (!word)
        {
            debug_print(2013, 9, "DEBUG: Failed to get delimiter word\n");
            shell_free(shell, token);
            return NULL;
        }
        
        debug_print(2013, 9, "DEBUG: Creating delimiter token: '%s'\n", word);
        delimiter_token = create_token(shell, word, TOKEN_WORD);
        shell_free(shell, word);
        
        if (!delimiter_token)
        {
            debug_print(2013, 9, "DEBUG: Failed to create delimiter token\n");
            shell_free(shell, token);
            return NULL;
        }
    ...
    if (!token)
    {
        debug_print(2013, 10, "DEBUG: Failed to create redirection token\n");
        return NULL;
    }
