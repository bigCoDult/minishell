...
/*   Updated: 2025/02/05 12:32:13 by yutsong          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char *get_path_value(t_shell *shell)
{
    ...
    printf("DEBUG: [get_path_value] Starting to get PATH value\n");
    
    if (!shell)
    {
        printf("DEBUG: [get_path_value] Shell is NULL\n");
        return (NULL);
    }

    if (!shell->env)
    {
        printf("DEBUG: [get_path_value] Shell env is NULL\n");
        return (NULL);
    }

    printf("DEBUG: [get_path_value] Searching through environment variables\n");
    current = shell->env;
    
    while (current)
    {
        printf("DEBUG: [get_path_value] Checking env: %s=%s\n", 
               current->key ? current->key : "NULL", 
               current->value ? current->value : "NULL");
        
        if (current->key && strcmp(current->key, "PATH") == 0)
        {
            printf("DEBUG: [get_path_value] Found PATH: %s\n", current->value);
            return (current->value);
        }
        current = current->next;
    }
    
    printf("DEBUG: [get_path_value] PATH not found in environment\n");
    return (NULL);
}
...
char *find_executable(t_shell *shell, const char *cmd)
{
    ...
    printf("DEBUG: [find_executable] Starting search for: %s\n", cmd);
    
    // 명령어가 절대 경로나 상대 경로인 경우
    if (cmd[0] == '/' || cmd[0] == '.')
    {
        printf("DEBUG: [find_executable] Command is a path\n");
        return (shell_strdup(shell, cmd));
    }

    // PATH 환경 변수 가져오기
    printf("DEBUG: [find_executable] Getting PATH value\n");
    path = get_path_value(shell);
    if (!path)
    {
        printf("DEBUG: [find_executable] PATH not found\n");
        return (NULL);
    }

    printf("DEBUG: [find_executable] PATH value: %s\n", path);
    path_copy = shell_strdup(shell, path);
    if (!path_copy)
    {
        printf("DEBUG: [find_executable] Failed to copy PATH\n");
        return (NULL);
    }

    printf("DEBUG: [find_executable] Searching in PATH directories\n");
    dir = strtok(path_copy, ":");
    
    while (dir)
    {
        printf("DEBUG: [find_executable] Checking directory: %s\n", dir);
        full_path = shell_malloc(shell, strlen(dir) + strlen(cmd) + 2);
        if (!full_path)
        {
            printf("DEBUG: [find_executable] Memory allocation failed\n");
            shell_free(shell, path_copy);
            return (NULL);
        }
        
        sprintf(full_path, "%s/%s", dir, cmd);
        printf("DEBUG: [find_executable] Trying path: %s\n", full_path);
        
        if (access(full_path, X_OK) == 0)
        {
            printf("DEBUG: [find_executable] Executable found at: %s\n", full_path);
            shell_free(shell, path_copy);
            return (full_path);
        }
        
        printf("DEBUG: [find_executable] Not found in this directory\n");
        shell_free(shell, full_path);
        dir = strtok(NULL, ":");
    }
    
    printf("DEBUG: [find_executable] Executable not found in PATH\n");
    shell_free(shell, path_copy);
    return (NULL);
}
